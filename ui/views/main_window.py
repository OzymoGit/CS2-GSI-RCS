...
class ControlPanel:
    ...
    def _create_status_section(self) -> QFrame:
        ...
        self.input_method_label = QLabel("Input Method: Checking...")
        self.input_method_label.setFont(QFont("Arial", 9))
        self.input_method_label.setMinimumHeight(20)
        layout.addWidget(self.input_method_label)
...
class MainWindow(QMainWindow):
    ...
    def _sync_initial_ui_state(self):
        ...
        try:
            if hasattr(self.recoil_service, 'input_service'):
                input_svc = self.recoil_service.input_service
                if input_svc.kmbox_enabled and input_svc.kmnet_available:
                    self.control_panel.input_method_label.setText("Input: kmBox Net")
                    self.control_panel.input_method_label.setStyleSheet("color: #4CAF50; font-weight: bold;")
                elif input_svc.kmbox_enabled and not input_svc.kmnet_available:
                    self.control_panel.input_method_label.setText("Input: kmBox (Error)")
                    self.control_panel.input_method_label.setStyleSheet("color: #f44336; font-weight: bold;")
                else:
                    self.control_panel.input_method_label.setText("Input: Win32API")
                    self.control_panel.input_method_label.setStyleSheet("color: #2196F3; font-weight: bold;")
    ...
    def _setup_ui(self):
        ...
        self.setWindowTitle("Kashira Kashira")
    ...
    def _on_settings_saved(self):
        ...
        rcs_enabled = features.get("rcs_enabled", True)
        if not rcs_enabled and self.recoil_service.active:
            self.logger.info("RCS disabled by Master Switch. Stopping compensation.")
            self._stop_compensation()
