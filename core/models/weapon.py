...
class WeaponProfile:
    ...
    def __init__(
        ...
        self.abs_points_x: List[float] = []
        self.abs_points_y: List[float] = []
    ...
    def _calculate_pattern(self) -> None:
        ...
        # Skip legacy static subdivision. The pattern will be dynamically smoothed
        # at runtime by the HumanizerEngine.
        self.calculated_pattern = self.recoil_pattern[:self.length]

        # Calculate absolute paths for the humanizer generator
        self._calculate_absolute_path()

        # Validation logging
        self._validate_subdivision_precision()

    def _calculate_absolute_path(self) -> None:
        """Pre-calculate absolute path with padding for Catmull-Rom interpolation."""
        if not self.calculated_pattern:
            self.abs_points_x = []
            self.abs_points_y = []
            return
        ...
        for p in self.calculated_pattern:
            curr_x += p.dx
            curr_y += p.dy
            abs_x.append(curr_x)
            abs_y.append(curr_y)

        # Pad for Catmull-Rom (needs 4 points minimum, pad start and end)
        if abs_x:
            self.abs_points_x = [abs_x[0]] + abs_x + [abs_x[-1]]
            self.abs_points_y = [abs_y[0]] + abs_y + [abs_y[-1]]
