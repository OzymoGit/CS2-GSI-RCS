...
class InputService:
    ...
    def __init__(self, config_service=None):
        ...
        self.kmnet_available = False
        self.kmbox_enabled = False
        if config_service:
            kmbox_config = config_service.config.get("kmbox", {})
            self.kmbox_enabled = kmbox_config.get("enabled", False)
            if self.kmbox_enabled:
                try:
                    import kmNet
                    ip = kmbox_config.get("ip", "192.168.2.188")
                    port = kmbox_config.get("port", "12345")
                    uuid = kmbox_config.get("uuid", "your-uuid-here")
                    self.logger.info(f"[KMNET] Connecting to {ip}:{port}...")
                    kmNet.init(ip, port, uuid)
                    kmNet.monitor(5001)
                    self.logger.info("[KMNET] Connected!")
                    self.kmnet_available = True
                except ImportError:
                    self.logger.error("Error: kmNet module not found! kmbox is enabled but module is missing. NO fallback.")
                except Exception as e:
                    self.logger.error(f"[KMNET] Error: {e}")
    ...
    def mouse_move(self, dx: int, dy: int) -> None:
        ...
        if self.kmbox_enabled:
            if self.kmnet_available:
                try:
                    import kmNet
                    kmNet.move(int(dx), int(dy))
                    if self.logger.isEnabledFor(logging.DEBUG):
                        self.logger.debug("KMNet Mouse moved: dx=%s, dy=%s", dx, dy)
                except Exception as e:
                    self.logger.error("KMNet move failed: %s.", e)
            return
    ...
    def is_key_pressed(self, vk_code: int) -> bool:
        ...
        if self.kmbox_enabled and vk_code == win32con.VK_LBUTTON:
            if self.kmnet_available:
                try:
                    import kmNet
                    is_pressed = (kmNet.isdown_left() == 1)
                    
                    if self._last_key_states.get(vk_code) != is_pressed:
                        if self.logger.isEnabledFor(logging.DEBUG):
                            status = "pressed" if is_pressed else "released"
                            self.logger.debug("KMNet Key VK_LBUTTON: %s", status)
                        self._last_key_states[vk_code] = is_pressed
                    return is_pressed
                except Exception as e:
                    self.logger.error("KMNet isdown_left failed: %s", e)
            return False
